    ...
    public async Task Alpha1SchemaToPhase42_PreservesDataAndConvertsLegacyChairCount()
    {
        ...
        try
        {
            var factory = new TestFactory(
                Path.Combine(root, "alpha1.db"));
            DateTime utc =
                new(2026, 7, 18, 12, 0, 0, DateTimeKind.Utc);
            await using (PeluqueriaDbContext context =
                         factory.CreateDbContext())
            {
                string alpha1 = context.Database.GetMigrations()
                    .Single(x => x.EndsWith(
                        "_CompleteAdministration",
                        StringComparison.Ordinal));
                await context.GetService<IMigrator>()
                    .MigrateAsync(alpha1, cancellationToken);
                ...
                Assert.Equal(
                    2,
                    await context.Chairs.CountAsync(cancellationToken));
            }
            ...
            var service = new AdministrationService(
                ...
                new FixedClock(utc));
            var person = LocalUsePerson.Create(
                "Persona",
            ...
            await service.AddAsync(
                MonthlyRestockPlan.Create(
                    product.Id,
                    new YearMonth(2026, 8),
                    Quantity.NonNegative(2),
                    utc),
                cancellationToken);
            await service.AddAsync(
            ...
            await service.UpdateCollaboratorFundParticipationAsync(
                ...
                cancellationToken);
            var close = await service.CloseMonthAsync(
                new YearMonth(2026, 7),
                new MonthlySummaryInput(
                    1000,
                    0,
                    0,
                    0,
                         ...
                         factory.CreateDbContext())
            {
                ...
                Assert.Equal(
                    1,
                    await context.Settings.CountAsync(cancellationToken));
                Assert.Equal(
                    2,
                    await context.Chairs.CountAsync(cancellationToken));
                Assert.Equal(
                    ["Silla 1", "Silla 2"],
                    await context.Chairs
                        .OrderBy(x => x.Name)
                        .Select(x => x.Name)
                ...
                Assert.True(
                    await context.LocalUsePeople.AnyAsync(
                        cancellationToken));
                GeneralSettings migratedSettings =
                    await context.Settings.SingleAsync(cancellationToken);
                ...
                Assert.True(
                    await context.Products.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.InventoryMovements.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.RestockPlans.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.FinancialEntries.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.Obligations.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.ObligationPayments.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.MaintenanceRecords.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.Collaborators.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.MonthlyCloses.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.MonthlyCloseParticipants.AnyAsync(
                        cancellationToken));
                Assert.True(
                    await context.DistributionPayments.AnyAsync(
                        cancellationToken));
                Assert.Equal(
                    0,
                    await context.FormDrafts.CountAsync(
                        cancellationToken));
                Assert.True(
                    await context.ActivityRecords.AnyAsync(
                        cancellationToken));
                Assert.Equal(
                    0,
