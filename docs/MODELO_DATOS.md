...
|---|---|---|
| Ajustes | `Settings`, `DailyRates`, `UnofficialExpenses` | Tarifa diaria vigente e histórica, porcentaje, moneda, carpeta de exportación y gastos recurrentes mensuales. La columna de tarifa semanal queda como almacenamiento compatible del valor actual. |
| Uso del local | `Chairs`, `LocalUsePeople`, `ChairAssignmentPeriods`, `DailyCharges`, `LocalUsePayments`, `WeeklyRates`, `WeeklyCharges` | Sillas, trabajadores, periodos históricos de asignación, cargos diarios exactos y pagos incluidos anticipos. Las tablas semanales quedan solo como legado y se suman sin convertirse. |
...
| Actividad | `ActivityRecords`, `FinancialEvents` | Registro no editable de operaciones visibles y eventos financieros con identificador de operación, entidad, valores anterior/nuevo, diferencia y estado. |
...
- Un cargo diario es único por trabajador y fecha, requiere asignación de silla activa, excluye domingos, conserva tarifa/importe históricos y vence el sábado correspondiente. La generación repetida es idempotente.
...
11. `Phase52DailyFeesContributionsAndBranding`: añade tarifas y cargos diarios, periodos de asignación y eventos financieros. Conserva tablas/columnas semanales, no convierte cargos históricos y, si no existe una fecha verificable para el valor vigente, inicia la nueva tarifa en la fecha de actualización y registra la limitación.
