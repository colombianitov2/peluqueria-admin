...
**Exportar toda la información a Excel** crea un único libro real `.xlsx` con nombre `PeluqueriaAdmin-AAAA-MM-DD_HH-mm-ss.xlsx` en la carpeta que el administrador haya configurado expresamente. Una instalación nueva no preselecciona el Escritorio ni ninguna otra ruta. Si el nombre ya existe, añade un sufijo y nunca sobrescribe el archivo anterior. La interfaz no ofrece exportación CSV múltiple.

La exportación usa una única fecha de corte y una lectura transaccional consistente. Incluye datos actuales, históricos y futuros ya conocidos, `Movimientos generales`, tarifas diarias, cargos diarios, periodos de asignación de silla, eventos financieros y tarifas semanales históricas de compatibilidad, deudas y compromisos técnicos, trazabilidad de cierres, exclusiones, lista mensual, compras vinculadas, préstamos/cuotas, obligaciones y créditos con su recurrencia, cierres mensuales/anuales, saldos arrastrados, distribuciones congeladas, `Notas`, inventario acumulado, eliminaciones lógicas y borradores separados. Las hojas visibles `Resúmenes mensuales` y `Balance anual` usan exclusivamente las categorías claras vigentes; las hojas técnicas conservan el detalle necesario para una copia completa. La hoja `Planes de reposición` conserva la estructura anterior y `Compatibilidad inventario` identifica los indicadores antiguos de la lista mensual; ninguno se presenta como una operación vigente. Los textos peligrosos para fórmulas se escriben como texto. Primero se crea un temporal y solo se mueve a la carpeta final cuando el libro está completo; ante una falla se elimina el temporal.
