    ...
    TimeProvider timeProvider) : ObservableObject
{
        ...
        try
        {
                ...
                .OrderByDescending(item => item.Date).ThenByDescending(item => item.CreatedUtc))
            {
                ...
                Sales.Add(new SaleHistoryRow(
                    ...
                    quantity == 0 ? string.Empty : $"{settings.CurrencyCode} {total / quantity:N2}",
                    $"{settings.CurrencyCode} {total:N2}",
    ...
    {
        CalculatedTotal = SelectedProduct?.DefaultPrice is decimal price
            && decimal.TryParse(QuantityText, NumberStyles.Number, CultureInfo.CurrentCulture, out decimal quantity)
    ...
    private async Task PersistDraftAsync(CancellationToken cancellationToken = default)
    {
        ...
        string json = JsonSerializer.Serialize(new SaleDraft(SelectedProduct?.Product.Id, SaleDate, QuantityText, DescriptionText));
    ...
    {
        bool valid = decimal.TryParse(value, NumberStyles.Number, CultureInfo.CurrentCulture, out decimal number)
    ...
    private sealed record SaleDraft(Guid? ProductId, DateTime? Date, string Quantity, string Description);
}
